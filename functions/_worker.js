...
async function addLedgerAndUpsertBalanceByUserId(sb, userId, symbol, delta, reason, refId) {
  const { error: ledgerErr } = await sb.from("token_ledger").insert({
...
// accept both with and without /api
const matchAny = (p, variants) => variants.some((v) => is(p, v));
...
export default {
  async fetch(request, env) {
    ...
    // GET /me/stamina (also /api/me/stamina)
    if (matchAny(path, ["/api/me/stamina", "/me/stamina"]) && request.method === "GET") {
      ...
      try {
        ...
        if (sel.data?.cap !== cap) {
          ...
          await supabase.from("stamina_states").upsert({ user_id: userId, stamina, cap }, { onConflict: "user_id" });
        }
        return json({
          wallet, user_id: userId, rank: rankName,
          stamina, cap,
          updated_at: sel.data?.updated_at, last_tick_at: sel.data?.last_tick_at,
        });
      } catch (e) {
    ...
    // POST /me/stamina/spend (also /api/me/stamina/spend)
    if (matchAny(path, ["/api/me/stamina/spend", "/me/stamina/spend"]) && request.method === "POST") {
      const wallet = getWalletLowerFromAny(request, url);
      if (!wallet) return json({ error: "Missing or invalid wallet" }, 400);
      ...
      try {
        ...
        const up = await supabase.from("stamina_states").upsert(
    ...
    if (matchAny(path, ["/api/health", "/health"]) && request.method === "GET") {
      const { error } = await supabase.from("heists").select("key").limit(1);
      if (error) return json({ ok: false, error: error.message }, 500);
      return json({ ok: true });
    }

    if (matchAny(path, ["/api/heists", "/heists"]) && request.method === "GET") {
    ...
    if (matchAny(path, ["/api/me", "/me"]) && request.method === "POST") {
    ...
    if (matchAny(path, ["/api/me/balances", "/me/balances"]) && request.method === "GET") {
      const wallet = getWalletLowerFromAny(request, url);
      if (!wallet) return json({ error: "Missing or invalid wallet" }, 400);
      try {
        const userId = await getOrCreateUserId(supabase, wallet);
    ...
    if (matchAny(path, ["/api/me/ledger", "/me/ledger"]) && request.method === "GET") {
      const wallet = getWalletLowerFromAny(request, url);
      if (!wallet) return json({ error: "Missing or invalid wallet" }, 400);
      try {
        const userId = await getOrCreateUserId(supabase, wallet);
    ...
    if (matchAny(path, ["/api/reward", "/reward"]) && request.method === "POST") {
    ...
    if (matchAny(path, ["/api/rewardBatch", "/rewardBatch"]) && request.method === "POST") {
    ...
    /* ---------- Admin (unchanged, but path-aliased) ---------- */
    if (path.startsWith("/api/admin/") || path.startsWith("/admin/")) {
      if (!isAdmin(request, env)) return json({ error: "forbidden" }, 403);

      const p = path.replace(/^\/api\//, "/");

      if (p === "/admin/resetAllBalances" && request.method === "POST") {
        ...
        } catch (e) { return json({ error: e.message || "Reset failed" }, 500); }
      }

      if (p === "/admin/resetWalletBalances" && request.method === "POST") {
      ...
      if (p === "/admin/addFunds" && request.method === "POST") {
        ...
        } catch (e) { return json({ error: e.message || "Add funds failed" }, 500); }
      }

      if (p === "/admin/grantStamina" && request.method === "POST") {
        ...
        } catch (e) { return json({ error: e.message || "Grant stamina failed" }, 500); }
      }

      if (p === "/admin/holdingsSummary" && request.method === "GET") {
        try {
          let rows, error;
        ...
        } catch (e) { return json({ error: e.message || "Summary failed" }, 500); }
      }
